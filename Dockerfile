# Single-container HF Space: Postgres 16 (pinned, per CLAUDE.md) + the FastAPI
# app in one image. No persistent volume on the free tier, so start.sh loads
# TPC-H fresh on every boot -- the project already treats DB state as
# disposable/reproducible (oracle.reset_indexes(), load_data.py is idempotent).
FROM postgres:16
